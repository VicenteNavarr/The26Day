...
public class MovingPlatform : MonoBehaviour
{
    ...
    private void Update()
    {
        // Si la distancia entre la posición actual de la plataforma y la posición A es menor que 0.05, cambia la posición objetivo a la posición B
        if (Vector2.Distance(transform.position, positionA.position) < 0.05f)
        {
            targetPos = positionB.position;
        }

        // Si la distancia entre la posición actual de la plataforma y la posición B es menor que 0.05, cambia la posición objetivo a la posición A
